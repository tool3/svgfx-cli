...
`svgfx logo.svg -p crt`

<img src="https://raw.githubusercontent.com/tool3/svgfx-cli/master/examples/svgs/crt.svg" width="420" alt="crt preset">
...
[![npm](https://img.shields.io/npm/v/@svgfx/cli)](https://www.npmjs.com/package/@svgfx/cli)
...
- ✅ **Pipe-first** — `cat logo.svg | svgfx -p vhs > out.svg` just works.
...
- ✅ **27 effects + 8 presets** — one binary, `svgfx list` to see them all.
- ✅ **Batch a folder** — `svgfx icons/ -p riso -o dist/`.
...
```bash
npm install -g @svgfx/cli
```

Or run it without installing:

```bash
npx @svgfx/cli logo.svg -p crt -o out.svg
```

## Quick start

```bash
svgfx logo.svg -e bloom                        # one effect, printed to stdout
svgfx logo.svg -p crt -o out.svg               # a preset, written to a file
cat logo.svg | svgfx -p vhs > out.svg          # piped in, redirected out
svgfx icons/ -p riso -o dist/                  # a whole directory
svgfx list                                     # everything available
```

Effects stack in the order you pass them, the same way they do in the library:

```bash
svgfx logo.svg -e "bloom:radius=8" -e "scanlines:gap=3" -e "grain:amount=0.3"
```

<img src="https://raw.githubusercontent.com/tool3/svgfx-cli/master/examples/svgs/stacked.svg" width="420" alt="bloom, scanlines and grain stacked">

## Input

| Form | Example |
| ---- | ------- |
| File | `svgfx logo.svg -p crt` |
| Several files | `svgfx a.svg b.svg -e invert -o dist/` |
| Glob (shell expands it) | `svgfx icons/*.svg -p riso -o dist/` |
| Directory | `svgfx icons/ -p riso -o dist/` |
| Piped stdin | `cat logo.svg \| svgfx -p vhs` |
| Explicit stdin | `svgfx - -e grain < logo.svg` |
| Remote URL | `svgfx https://example.com/logo.svg -p neon` |
...
`svgfx list` prints all of these with their options.
...
```bash
svgfx logo.svg -e "halftone:size=5,angle=15,keep-source=true"
svgfx logo.svg -e "glow:color=#ff2d55,radius=8"
svgfx logo.svg -e "grain:animate"              # a bare key means true
```

<img src="https://raw.githubusercontent.com/tool3/svgfx-cli/master/examples/svgs/halftone.svg" width="300" alt="halftone"> <img src="https://raw.githubusercontent.com/tool3/svgfx-cli/master/examples/svgs/glitch.svg" width="300" alt="glitch">
...
```bash
svgfx terminal.svg -p crt -o out.svg              # rounded corners survive
svgfx terminal.svg -e scanlines:clip=viewport   # opt out, fill the whole frame
```
...
<img src="https://raw.githubusercontent.com/tool3/svgfx-cli/master/examples/svgs/riso.svg" width="300" alt="riso preset"> <img src="https://raw.githubusercontent.com/tool3/svgfx-cli/master/examples/svgs/neon.svg" width="300" alt="neon preset">

Presets take options too:

```bash
svgfx logo.svg -p "riso:shadow=#2b3a67,highlight=#ff5a5f"
svgfx logo.svg -p "crt:animate=true"
```
...
| `-s`, `--seed` | `svgfx` | Seed for every random decision |
| `--prefix` | `svgfx` | Prefix for generated ids |
...
```bash
svgfx logo.svg -c poster.json -o poster.svg
```

**Straight into CSS**

```bash
echo "background-image: url('$(svgfx logo.svg -p crt -u)');"
```

**Every icon in a folder, minified**

```bash
svgfx icons/ -p riso -f minify -o dist/
```

**An SVG that already animates keeps animating**

```bash
svgfx spinner.svg -p crt -o spinner-crt.svg
```

<img src="https://raw.githubusercontent.com/tool3/svgfx-cli/master/examples/svgs/motion-crt.svg" width="300" alt="crt over an animated svg">

**Reproducible output in CI**

```bash
svgfx logo.svg -e glitch --seed release-42 -o out.svg
```

## Library

This is the terminal front end for
[`@svgfx/postprocessing`](https://www.npmjs.com/package/@svgfx/postprocessing).
